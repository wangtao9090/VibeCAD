...
> 状态：AR-1 reviewed；S3-7/P0-A 已本地提交，S3-8 是当前唯一 active delivery packet
>
> 日期：2026-07-22
>
> 当前基线：VibeCAD 0.4.0 源码中的 Task Kernel TK1–TK9 与 S3-7/P0-A 已提交并通过本地
> 全量、受管 FreeCAD 和 packed MCPB 门禁；20-tool 公共 MCP、durable review、首批六操作和
> FCStd/STEP artifact 已实现但尚未 push/tag/release。S3-8 将完成宿主发现、skill、ResourceLink、
> 0.5.0 版本与最终分发验收；P0-B 和 FreeCAD Workbench 尚未交付。
...
1. S3-8 修复 tool description/namespace/discovery budget，交付 host-neutral skill、标准 ResourceLink、
   0.5.0 和最终 packed MCPB/FreeCAD E2E；
2. P0-B core 补 task/project/revision/artifact 可发现与可恢复能力、secure daemon/file grant、checkout
   source liveness/revocation 和最小 FreeCAD crash/hang isolation；
...
| 通用 Agent / 国内首批试点 | 腾讯 QClaw、WorkBuddy 个人版 | 用户形态最匹配低频个人 CAD；本地执行、Skill/MCP 能力有公开依据，但必须真机核对完整协议 | macOS 安装、20-tool discovery、resource read、审核和重启恢复全部通过 |
...
S3-7 代表 P0-A 实现完成；S3-8 才代表 Stage 3 具备可分发、协议/包层 host-ready 的闭环；真实
Claude/Codex 宿主验证仍受 S3-RES-06 约束。G1 的界面原型可以与 P0-B hardening 并行，但 P1
作为可交付产品完成前必须关闭 P0-B。

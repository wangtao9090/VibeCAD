...
> read-only artifact manifest 已实现
>
> 日期：2026-07-22
>
> 当前基线：VibeCAD 0.5.0 / runtime epoch 4 的 Task Kernel TK1–TK9、26-tool 公共 MCP、durable
...
> transition 审计、当前 HEAD 的 project/revision 发现、文件级版本比较与只读交付清单已进入当前分支候选。真实 Claude/Codex 主机尚未安装激活验收，当前没有 tag 或
...
当前近期缺口已经转为 P0-B core 的 task cancel、verified forward revert、daemon 和认证 IPC，
P0-B hardening 的 retention/GC，以及 G1 的 Workbench candidate review。
...
| 通用 Agent / 国内首批试点 | 腾讯 QClaw、WorkBuddy 个人版 | 用户形态最匹配低频个人 CAD；本地执行、Skill/MCP 能力有公开依据，但必须真机核对完整协议 | macOS 安装、26-tool discovery、resource read、审核和重启恢复全部通过 |

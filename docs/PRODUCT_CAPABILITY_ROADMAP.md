...
> P0-B core 正在执行，keyed task creation、task/project/revision discovery、revision comparison 与
> read-only artifact manifest、durable idle cancellation 已实现
>
> 日期：2026-07-23
>
> 当前基线：VibeCAD 0.5.0 / runtime epoch 4 的 Task Kernel TK1–TK9、27-tool 公共 MCP、durable
> review、首批六操作、host-neutral skill 与 FCStd/STEP ResourceLink 的当前分支候选。S3-8/P0-A
> 基线已通过本地协议、受管 FreeCAD 和 packed MCPB 门禁；`create_task` keyed replay、`list_tasks`
> 恢复发现、`get_task_events` transition 审计、当前 HEAD 的 project/revision 发现、文件级版本比较、
> 只读交付清单与空闲任务持久取消已进入后续源码候选。C05 不提前宣称完成 C14 的 0.6.0 package
> refresh。真实 Claude/Codex 主机尚未安装激活验收，当前没有 tag 或 release；P0-B 其余能力和
> FreeCAD Workbench 尚未交付。
...
当前近期缺口已经转为 P0-B core 的 active CAD cancellation/Worker kill 与 reconcile、verified
forward revert、daemon 和认证 IPC，P0-B hardening 的 retention/GC，以及 G1 的 Workbench
candidate review。空闲任务的 durable cancel 已由 C05 交付，不能把它与 active Worker termination
混为一项能力。
...
2. **[正在执行] P0-B core** 已补 keyed task、task/project/revision discovery、文件级 comparison、
   read-only manifest 和 idle cancel；继续实现 verified forward revert、active cancel reconcile、
   secure daemon/file grant、checkout source liveness/revocation 和最小 FreeCAD crash/hang isolation；
...
| 通用 Agent / 国内首批试点 | 腾讯 QClaw、WorkBuddy 个人版 | 用户形态最匹配低频个人 CAD；本地执行、Skill/MCP 能力有公开依据，但必须真机核对完整协议 | macOS 安装、27-tool discovery、resource read、审核和重启恢复全部通过 |
...
- `list/cancel task`、task request-key/recover、`list/compare/revert revision`、artifact manifest；其中
  discovery、request replay、file-level compare、manifest 和 idle cancel 已完成，verified forward
  revert 与 active cancellation reconcile 仍在 P0-B 后续 packet；
...
| 版本与恢复 | `revert_project`；`cancel_task` 的 active Worker reconciliation | P0 | verified forward revision、killable Worker、CAS、审计事件 |

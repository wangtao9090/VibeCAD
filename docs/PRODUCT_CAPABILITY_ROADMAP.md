...
2. **参数类型系统**：当前只有 string、positive number、boolean、vector3，无法表达
   enum、vector2、selector、profile、pattern、单位量和严格复合结构；
3. **稳定选择器**：进程内 `Face6/Edge8` 和当前标签快照不能跨 recompute、reload
   与 Revision；
4. **细粒度 observation/verifier**：当前只能证明整体 volume、area、bbox、质心、
...
- 捕获用户在画布中选中的 part/feature/face/edge，生成 SelectorV1；
- 在独立 Preview Document 中展示 candidate 和前后差异；
- Accept、Reject、Revise，不先提交再伪装成撤销；
- 将用户手工修改封装成新的 candidate checkpoint；旧 draft verdict 立即失效，经
  Kernel 重新观察和验证后才能发布为新 Revision；
- 定位、高亮和比较 Agent 所引用的几何目标。
...
G0/Stage 3 先实现只覆盖 object/feature UUID、revision、semantic role、provenance 和
cardinality 的 Level A；mapped subobject、face/edge fingerprint、adjacency 和 pick
context 在 G1/P1 的 Level B 完成。

### 5.4 插件分期

| 阶段 | 范围 | 用户可见结果 |
|---|---|---|
| G0 | `CadExecutionPort`、durable draft/review、managed checkout、IPC protocol、SelectorV1 Level A | 内核具备接入 GUI 的正确语义 |
| G1 | local Kernel daemon、认证 IPC、Python Workbench、Dock、加载 candidate、Accept/Reject、选择捕获 | 用户可以在 FreeCAD 中看见并验收 Agent 修改 |
| G2 | GUI worker、TaskPanel 参数微调、实时预览、手工 checkpoint/publish | 用户与 Agent 可以围绕同一模型协同精调 |
...
发布矩阵成本。G0 只冻结 Application API 与 IPC protocol；Stage 3 期间 MCP server
仍可在进程内拥有 AgentApplication。G1 再把相同 API 放入独立本地 Kernel daemon，
插件通过经过认证的本地 IPC 连接，FreeCAD GUI 操作通过 Qt queued signal 回到主线程。
...
- per-entity observation、preservation verifier、revision diff；
- BRep 和布尔输入验证，candidate-only healing；
- `list/cancel task`、`list/compare/revert revision`、artifact manifest；
- 原 31 工具中稳定能力的第一批 Agent 化；
- G0 插件协议和 durable review 状态；
- 同进程全局 CAD gate、超时、预算和恢复边界。
...
- P0-A / Stage 3：首批控制面、六个 object-level operation、直接工具派生、
  result handle、SelectorV1 Level A、细粒度 verifier、durable review 和 G0 protocol；
- P0-B：list/cancel task、list/compare/revert revision、revision diff、完整 artifact
  manifest，以及 G1 需要的 daemon/认证 IPC 产品化。

S3-R3 完成代表 P0-A 完成，不代表整个 P0 完成。G1 的界面原型可以在 G0 后并行开始，
但 P1 作为可交付产品完成前必须关闭 P0-B。

### P1 — 交互式单零件设计与 STL 主流程

新增或迁移：

- G1/G2 FreeCAD Workbench；
...
| 验证与修复 | `validate_geometry`、`diagnose_boolean`、`propose_geometry_repair` | P0/P1 | OCCT BRepCheck/Shape Healing、candidate-only |

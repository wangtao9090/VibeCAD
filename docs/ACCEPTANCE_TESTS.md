...
本清单验证当前 Agent-first 产品：持久化 Project/Task/Revision/Draft/Artifact、24 个公开工具、direct
...
运行时 `tools/list` 与 MCPB manifest 必须同序公开以下 24 个唯一名称：
...
| G02 | 公开面 | 精确 24 个唯一工具；说明与 manifest 完全一致；固定 discovery frame ≤ 32,768 bytes | ☐ |
...
- 正文列出精确 24 个工具，先 `get_capabilities`，包含 project/task/review/artifact 流程；
...
1. initialize、24-tool discovery、resource template；

...
|---|---|---|---|---|
| A1 | 握手 | "先 ping 一下 VibeCAD，告诉我现在有哪些 CAD 工具可用。" | 连接器列表见 vibecad；ping 回 `vibecad ok (v…)`；能列出 22 个工具 | ☐ |
...
- **前置**：已在 Cowork 中配置好 vibecad 连接器并连接（配置方法见 B 部分第 4 步或用户手册）。
- **你说**："先 ping 一下 VibeCAD，告诉我现在有哪些 CAD 工具可用。"
- **预期**：
  - Cowork 连接器列表中可见 **vibecad**，状态正常；
  - `ping` 返回 **`vibecad ok (v…)`**（版本号随发布变化）；
  - AI 列出的工具应包含 `ensure_runtime`、`smoke_cad`、`add_box`、`add_hole`、`modify_part`、`align_parts`、`export_part` 等，全量 **22 个**。
- **若失败**：连接器未配置成功 / uv 不在 PATH / command 或 args 拼写错——收集 Cowork 连接器配置截图，并在终端单独跑 `uvx --from git+https://github.com/wangtao9090/VibeCAD vibecad` 记录报错输出。
...
- Claude Cowork 已安装并登录

### 验证步骤（六步）

1. **安装 uv**——PowerShell 中执行：

   ```powershell
   powershell -ExecutionPolicy ByPass -c "irm https://astral.sh/uv/install.ps1 | iex"
   ```

2. **验证 uv**——**重开一个**终端窗口（让 PATH 生效），执行 `uv --version`，能打印版本号即过。

3. **握手验证**——终端执行：

   ```powershell
   uvx --from git+https://github.com/wangtao9090/VibeCAD vibecad
   ```

   首次会拉取源码并构建（约 1 分钟）。随后进程**安静地挂住不退出、无报错**——这是 stdio 模式在等客户端握手，即为成功；按 `Ctrl-C` 退出。
   （PyPI 发布后此步可简化为 `uvx vibecad`。）

4. **Cowork 配置**——添加自定义连接器 / 本地 MCP server（stdio 类型）：
   - command：`uvx`
   - args：`--from git+https://github.com/wangtao9090/VibeCAD vibecad`（PyPI 发布后改为 `vibecad`）

   具体 UI 字段名与入口位置**以 Cowork 当前版本实际界面为准**。

5. **确认握手**——重启 Cowork（或重连连接器），确认连接器列表中出现 **vibecad** 且状态正常（即 A1 的判定标准）。

6. **跑五条核心场景**——按 A 部分原话依次执行 **A2 → A4 → A5 → A9 → A11**，逐条勾选。注意：

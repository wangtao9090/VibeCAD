...
> 版本：Round 11（0.3.0）。对应实现状态：R1 运行时 → R8 装配 DSL → R11 自动安装 + 零重连换芯 + 卸载，共 23 个 MCP 工具。
> 配套阅读：[`README.md`](../README.md) 工具表、[`docs/USER_GUIDE.md`](USER_GUIDE.md) 用户手册。

## 使用说明

- **怎么跑**：建议按 A1 → A13 顺序执行（场景间有剧本依赖，见「剧本分组」）。每完成一条，在总表「通过」列打勾 ☑。
- **可中断续跑**：A1–A3（环境三条）只需跑一次；A4 起每组场景开头都会新建文档，从任一组的第一条（A4 / A7 / A8 / A9）都能重新接上，不必从头再来；A13（卸载）建议放在其余场景全部跑完后再做，因为会清空运行时。
...
| 卸载 | A13 | 无 | 两段式确认删引擎 + CLI 救援命令，放最后跑（会清空运行时） |

### 验收总表

| 编号 | 场景 | 你说的话（可照念） | 预期结果（详见细则） | 通过 |
|---|---|---|---|---|
| A1 | 安装与握手 | （双击安装 `VibeCAD.mcpb` 后，**不额外说任何安装指令**）"先 ping 一下 VibeCAD，告诉我现在有哪些 CAD 工具可用。" | 设置→扩展可见 VibeCAD 且已启用；ping 回 `vibecad ok (v…)`；新对话能列出 23 个工具 | ☐ |
...
| A13 | 卸载 | 对话里说"把 CAD 引擎卸载了"（预览）→ 确认 → 观察目录；另跑一次命令行 `uvx vibecad --uninstall --yes` | 第一次调用只预览路径与大小、不删除；确认后**当场自动完成删除**（无需手动重启），运行时目录消失；CLI 命令同样能清空目录且退出码为 0 | ☐ |
...
- **预期**：
  ...
  - 新对话中 AI 列出的工具应包含 `ensure_runtime`、`get_runtime_status`、`smoke_cad`、`add_box`、`add_hole`、`modify_part`、`align_parts`、`export_part`、`uninstall_runtime` 等，全量 **23 个**。
- **若失败**：
...
3. **重启确认握手**——完全退出并重启 Claude Desktop / Cowork，新开对话，按 A1 原话 ping 一次，确认能列出 23 个工具（即 A1 的判定标准）。

...
本清单验证当前 Agent-first 产品：持久化 Project/Task/Revision/Draft/Artifact、27 个公开工具、direct
...
运行时 `tools/list` 与 MCPB manifest 必须同序公开以下 27 个唯一名称：
...
- 成功交付只有 FCStd 与 STEP；
- `cancel_task` 当前只立即取消空闲任务；active FreeCAD Worker termination/reconcile 属于 C12；
...
| G02 | 公开面 | 精确 27 个唯一工具；说明与 manifest 完全一致；固定 discovery frame ≤ 32,768 bytes | ☐ |
...
| G10 | 数据保护 | runtime uninstall 与 idle cancel 不删除/改写项目数据；执行和导出不污染源文件或暴露任意路径 | ☐ |
...
3. private server epoch 为 4，public-surface digest 绑定 description、input/output enforcement schema 与
   annotations；当前 canonical projection 为 426,508 bytes，SHA-256 为
...
当前 C05 固定 SDK projection 应为 20,717 bytes，SHA-256 为
`57a38baa2bb79d959037d3066e68468066893b01383cfdd8f77dac447d79e9e8`；计入 JSON-RPC envelope
和末尾 LF 的完整 frame 应为 20,762 bytes，SHA-256 为
...
- 正文列出精确 27 个工具，先 `get_capabilities`，包含 project/task/review/artifact 流程；
- direct/ModelProgram、SelectorV1、AcceptanceSpec、ResultRef、generation 与恢复表和实际 schema 一致；
- cancellation 段明确 idle-only、同请求重放、review 使用 reject、active C12 边界和
  `notifications/cancelled` 的 transport-only 语义；`cancel_requested` / `cancelling` 在 C12 前不得
  路由到 `resume_task`；
...
3. stale/future generation、未知 task、active state、`recovery_required` / `cleanup_required`、
...
5. 对预置的 `cancel_requested`、`cancelling`、`cancelled` 只验证可跨重启读取和幂等返回；前两种状态
   不得被写成 C05 已能从 active execution 生成或完成；
...
8. active FreeCAD hang/kill/未提交证明与最终 cancel reconcile 留给 C12 gate，本节不得据此宣称
   active cancellation 已交付。
...
1. initialize、27-tool discovery、resource template；
...
7. malformed/oversize/unknown-name/no-secret 负例；
8. restart 后项目、task 和资源仍存在。

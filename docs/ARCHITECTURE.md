...
当前 `tools/list` 精确包含 22 个工具：16 个稳定控制/领域 facade，加 6 个 registry-derived 直接
CAD 工具。
...
保留完整输出验证；当前固定 22-tool discovery frame 不超过 32,768 bytes。direct operation 与稳定工具
重名会 fail closed。
...
- 源码/manifest/lock/受管 receipt 版本：0.5.0；公开工具：22；MCP 1.27.2；受管 server epoch 4；
  FreeCAD 1.1.0；public-surface digest 为
  `a8b31d42abc4ece89d5f6a46a19912520c54ab64d27a5ecc53cb218e10caf5af`；
- 固定 discovery frame 不超过 32,768 bytes，tool description 和 input schema 对宿主可见，完整 output
  validation 保留在服务端；
...
发现、长任务和文件取回尚未执行；当前没有 tag 或 release。
...
| `manifest.json` | MCPB 平台、启动和 22-tool 静态声明 |

...
> 实现基线：Stage 3 S3-7，语义提交 `b822fc5914fabe3d7ee4924dfcdce14e08f04ba7`
>
> 架构复审：AR-1 / 2026-07-21
>
> 当前源码版本仍为 0.4.0；S3-8 完成宿主 skill、发现合同、文档、版本和协议/包验收后，
> 统一切换为目标版本 0.5.0。当前本地提交尚未 push、tag 或发布。
>
...
macOS 默认根目录为 `~/Library/Application Support/VibeCAD`。S3-7 的 durable Application data opener
...
当前 `tools/list` 精确包含 20 个工具：14 个稳定控制/领域 facade，加 6 个 registry-derived 直接
CAD 工具。
...
Manifest、运行时 discovery 和 receipt digest 都来自同一 public-surface 合同；S3-8 还会门禁 manifest
与 `PublicToolSpec` 的名称和 description 精确一致。所有 tool 输入是关闭的
...
AR-1 发现 S3-7 discovery 仍缺少 tool description，而且重复广播完整 task output schema，使一次
`tools/list` 约 350 KB。S3-8 必须在不削弱服务端输出校验的前提下补描述、压缩宿主发现投影，并
拒绝 direct operation 与稳定工具重名；这属于分发可用性修正，不改变 20-tool 业务边界。
...
└── AgentApplication                 process-owned composition root
    ...
    ├── ArtifactApi / ArtifactMaterializationService
    ├── TaskCatalogService
...
快照和内容寻址 artifact 元数据；它当前不提供项目列表或 revision 历史列表。
...
`create_task` 当前没有 caller request key；若其响应结果未知，会留下容量受限但无法按 ID 找回的孤儿
任务。`list_tasks`、request-key/recover、cancel/events 属于 P0-B，因此 S3-8 skill 必须禁止把未知结果
的 `create_task` 当成普通幂等调用盲重试。
...
上限固定为 64 MiB；更大文件、流式传输和本地 broker 属于 G1/P1。S3-8 还必须在成功 export 的 tool
result 中增加标准 ResourceLink，并用 typed/raw client 验证两个资源的发现、读取与保存；真实 Claude/
Codex 文件体验在未获外部模型调用授权前保留 residual，不能用任意用户路径 copy-out 绕过
ArtifactStore。
...
- 公开工具：20；MCP 1.27.2；受管 server epoch 3；FreeCAD 1.1.0；
- MCPB：Darwin；`uv.lock` 随包；tests/docs/runtime/cache 不进入产品包；
- wheel、sdist、MCPB 与 checkout 的 73 个 Python 文件 byte-exact；
- 默认非 slow 回归：3827 passed，95 deselected；
- 真实受管 Agent-first matrix、fresh packed MCPB、FCStd/STEP reload/resource read 均通过；
- 三路最终审查 Critical/Important/Minor 为 0/0/0；本地提交未 push。

S3-8 会因补 tool discovery/ResourceLink、版本和 skill 而重新生成 public-surface digest、递增私有 server
epoch、同步受管包并从 clean output root 重做全部 package/real-E2E 门禁。不能复用 S3-7 archive hash
冒充最终 0.5.0 产物。
...
| `manifest.json` | MCPB 平台、启动和 20-tool 静态声明 |
...
- host-neutral skill、协议/包层文件取回验收，以及仍待授权的真实 Claude/Codex 宿主验收；
- project/task/revision list、task request key/cancel/events、compare/revert 和 retention/GC；
...
固定推进顺序是：S3-8 host-ready 协议/包闭环 → P0-B 可发现/可恢复和 G1 后端前置 → G1 Workbench
MVP → P0-B hardening 收口 → P1/G2 单零件与 STL → P2 装配/图纸/交付。只有阶段需要改变专家

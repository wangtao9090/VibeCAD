...
当前 `tools/list` 精确包含 26 个工具：20 个稳定控制/领域 facade，加 6 个 registry-derived 直接
CAD 工具。
...
保留完整输出验证；当前固定 26-tool discovery frame 不超过 32,768 bytes。direct operation 与稳定工具
重名会 fail closed。
...
- 源码/manifest/lock/受管 receipt 版本：0.5.0；公开工具：26；MCP 1.27.2；受管 server epoch 4；
...
| `manifest.json` | MCPB 平台、启动和 26-tool 静态声明 |
...
- task cancel、verified forward revert 和 retention/GC；

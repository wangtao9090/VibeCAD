...
保留完整输出验证；当前固定 20-tool discovery frame 为 16,640 bytes。direct operation 与稳定工具
重名会 fail closed。这属于分发可用性修正，不改变 20-tool 业务边界。
...
任务。`list_tasks`、request-key/recover、cancel/events 属于 P0-B，因此当前 canonical skill 禁止把
未知结果的 `create_task` 当成普通幂等调用盲重试。
...
- 源码/manifest/lock/受管 receipt 版本：0.5.0；公开工具：20；MCP 1.27.2；受管 server epoch 4；
  FreeCAD 1.1.0；public-surface digest 为
  `84b6abe8c1b496153ed2be083e1ea3186f642c47d9dfa9c2f90f66e92e6139f9`；
- 固定 discovery frame 为 16,640 bytes，tool description 和 input schema 对宿主可见，完整 output
  validation 保留在服务端；
...
- checkout、wheel、sdist、MCPB 与安装结果的 Python 文件，以及各分发渠道的 skill 文件，已经完成
  byte-exact 审计；
- 默认非 slow 回归、真实受管 Agent-first matrix、Task Kernel candidate、fresh packed MCPB、
  FCStd/STEP reload、ResourceLink/resource read 均通过本地门禁。

这些事实只证明本地源码、协议和分发包 `host-ready`。真实 Claude/Codex 主机中的 skill 安装、reload、
发现、长任务和文件取回尚未执行，且当前候选没有 push、tag 或 release。

...
### 3.5 视觉反馈落盘（Cowork 真机新发现，2026-06-12）

**问题**：Cowork 不向用户渲染工具返回的 ImageContent——AI 能看到工程图（可描述内容），
用户展开工具结果也只见 "Result" 文本标签。产品核心价值（每步看图迭代）在 Cowork 上
对用户不可见；AI 试图自救时发现无落盘接口，只能 osascript 绕路。

**设计**：

- **每步自动附图同时落盘**：`_attach_view` 把工程图 PNG 写到
  `vibecad_home()/views/<doc>/<NNN>-<tool>.png`（滚动保留最近 20 张/文档），
  返回 dict 增加 `view_file` 字段（绝对路径）。落盘失败不连坐（与 render_error 同理）。
- **`render_part` 加 `save_to: str | None = None` 参数**：显式保存到 AI 指定路径
  （如 Cowork 工作目录），返回带 `saved` 字段。
- **AI 引导**：工具描述注明"Cowork 等不显示内嵌图的客户端，可用 view_file 路径
  `open <path>` 给用户弹图，或复制到工作目录预览"。
- Claude Desktop 聊天界面（内联显示 ImageContent，R3 已验证）不受影响，新增字段无害。
...
- **慢测**：真机换芯端到端（bootstrap 起 → 装好 → 自动换芯 → smoke_cad 过）；
  卸载后目录不存在；附图返回含 view_file 且文件存在。
- **真机验收**：测试 Mac 全新安装 → 自动下载 → 零重连画图（Cowork 上用户能看到
  落盘的工程图）→ Remove extension 全删。

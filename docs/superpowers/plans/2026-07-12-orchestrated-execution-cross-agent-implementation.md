...
- Create: `/Users/wangtao/.agents/skills/orchestrated-execution/scripts/validate_portability.py`
...
**技术勘误：** validator 文件名统一为可导入的 `validate_portability.py`。原连字符命名与测试接口 `from scripts.validate_portability import validate_skill` 不相容，会导致实现存在后仍无法转绿；不得以镜像文件、软链接或 wrapper 制造双真源。
...
def write_valid_package(root: Path) -> None:
    ...
    (root / "scripts/validate_portability.py").write_text(
        "# fixture\n", encoding="utf-8"
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if len(arguments) != 1:
        print("Usage: validate_portability.py SKILL_DIR", file=sys.stderr)
        return 2
...
```bash
python3 -m unittest discover -s tests -p 'test_*.py' -v
python3 -m py_compile scripts/validate_portability.py tests/test_validate_portability.py
...
```bash
git add scripts/validate_portability.py tests/test_validate_portability.py

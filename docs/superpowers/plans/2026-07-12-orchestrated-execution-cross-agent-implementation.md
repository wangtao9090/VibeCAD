...
| G-3 | 本机没有 OpenClaw runtime | 用户提供可运行的 OpenClaw 环境，或另行明确授权安装；未取得 runtime 前只允许静态兼容验证 | 阶段保持未关账，不继续 VibeCAD Agent |

G-1 与 G-2 属于本计划对已批准规格的明确实施补充；批准本计划即接受这两个补充。G-3 不自动授权安装未知软件。
...
- Produces: 两仓库安全锚点、唯一 `RUN_ROOT`、已验证的模型/runtime 可用性表。
...
Expected report fields: `backup_authorized`、`private_remote_authorized`、`openclaw_runtime_source`、`vibecad_head`、`legacy_hash_manifest`。任一授权为空即停止；不得开始 Task 1。
...
- [ ] **Step 4: OpenClaw runtime smoke**

在用户提供或另行授权安装的 OpenClaw 环境中，用同一 prompt 开新会话，并保存版本、发现路径、canonical commit 与最终输出。当前机器未安装 OpenClaw，因此在获得真实 runtime 前本步骤保持未完成，禁止 V4、禁止关账。

- [ ] **Step 5: 统一评分与提交 V4**

四份输出逐项验证：同源 commit、正确 adapter、四字段 profile、repo truth、无幻觉工具、缺能力可回退、批准门禁存在。两道审查通过后：
...
**Interfaces:**
- Consumes: V1–V4、S0–S4.n、四宿主 runtime evidence。
...
Expected: `N/N` 明确关账后，才把工作流切回 VibeCAD Agent Harness；OpenClaw smoke 未完成时不得执行本步骤。
...
- **Spec coverage:** D-1→Task 2/7；D-2→Task 3–5；D-3→Task 4/5；D-4→Task 1/4/6；D-5→Task 4/6；D-6→Task 1/4/6；D-7→模型路由/Task 5；D-8→Task 1/4/6；D-9→Task 5。安全、熔断、迁移和四宿主验收分别由 Task 0、7、8、9 覆盖。
...
- **Scope:** Skill 与 evidence 分属两个 Git 仓库；VibeCAD 产品代码、宿主配置和 OpenClaw 安装均不在隐式修改范围。

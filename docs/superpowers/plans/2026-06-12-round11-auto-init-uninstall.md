...
- Q2 自退后宿主行为（D 合格线）：**是（⚠️ 2026-07-03 真机推翻预判）**——用户 8 轮自退循环：每次 exit(0) 后宿主均自动拉起新进程（pid 单调递增 44042→44649），8/8 无一失败，复活对调用方近乎透明；其中一轮复活还顺带完成了 0.0.1→0.0.2 热升级。与源码推读（onclose 无 relaunch）不符，机制未定论（可能 exit-0 干净退出与 crash 分路径，或宿主版本已更新行为）。**对实现零影响，C 分支保留不回退 D**：supervisor 接住 exit(75) 后宿主根本看不到进程死，换芯协议（握手重放/pending 重发/循环护栏）全在自己手里更可控；Q2=是 反而额外白拿一层保险——supervisor 自身若意外崩溃，宿主会重新拉起整个 launcher 链。
- Q3 升级数据保留：**修正为"稳态持久，仅首次复活一次性清理"（✅ 2026-07-03 真机 8 轮实测）**——runtime/ 目录 7/8 轮保留（普通复活 6/7 保留、热升级那轮也保留）；唯一清空发生在首次复活，最可信解读为一次性首启初始化清理，非"重启即清"或"升级即清"规则。首轮观察到的"升级后 mark 消失"实为该首次清理，误归因于升级。开放观察项（无实际影响，不再实验）：首次清理确切触发条件需宿主冷启动对照。**核心决策不变且获真机支撑：既然扩展目录内至少存在一条会清数据的路径，VIBECAD_HOME 放扩展目录外（`~/Library/Application Support/VibeCAD`）就是正确选择——VIBECAD_HOME 不进 manifest env，Task 5 env 只留 VIBECAD_AUTO_INSTALL（已落实现）。**

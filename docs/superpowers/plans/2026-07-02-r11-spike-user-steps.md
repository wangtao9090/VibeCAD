...
> 背景：Round 11 的换芯（Task 3）与 manifest 定稿（Task 5）被 Task 0 spike 硬门阻塞，
> 三个问题（Q1/Q2/Q3）必须在真机 Claude Desktop 上得出结论才能解锁后续全部任务。
> spike 扩展包已打好，本文是照做即可的操作步骤。做完把三个结论告诉 Claude 即可（会代为回填计划文档并 commit）。

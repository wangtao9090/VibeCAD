...
**执行方式**：闭关 subagent-driven（架构轮）。Spike 四疑点一次通过（App::Part 容器选定）。

### 验收结果
- 快测 **300 passed**（含兼容硬线：R7 既有 227 条零修改）+ ruff 清零；真机慢测 **65 passed**；黑盒真协议全链路（四角孔阵→new_part→归属标签表→gap=0 贴面→干涉拒→split STEP）+ **装配工程图人眼确认**（两零件分色、被盖孔正确转虚线）。

### 第八轮审查战果（黑盒 2 bug + 终审两路 5 CRITICAL + 2 Important + 2 Medium，全部真机实锤修复）
1. **黑盒逮 bug**：装配标注（全局指纹）与解析（局部匹配）坐标系错位——零件被移动后刚标注的标签也匹配不上 → `_match_shape` 全局匹配/局部消费契约（`6f85d53`）；annotate 表无零件归属。
2. **系统性根因（3 CRITICAL 同源）**：装配守卫锚定"活动零件"而非"被操作零件"——active=B 时移 A 的孔刀具吞孔报 ok / sealed 探针跨零件坐标混用双向毒害 / 差集归属时序使断言跑在归属前（装配 pad 崩溃+多 solid 漏检）→ **owner 反查锚定统一修复**（`9394a95`）。
3. 干涉守卫 `except→vol=0` 静默放行（绝不静默纪律在最关键守卫上的破口）→ 删宽抓+空零件显式注明（`290d8fe`）。
4. edges_of 装配索引错位（画错零件的边）→ 全局索引映射；标签命名空间死锁（重标注只刷活动零件——假提示死循环）→ 全零件写入（`290d8fe`）。
5. 顺手：两阶段归属/慢测 allow 残留清理/describe 拉齐（`f7c8ede`）。

### 方法论沉淀（第八轮）
**"守卫必须锚定被操作对象的 owner，而非全局/活动状态"**——多上下文（多零件）架构下，一切按"当前上下文"取数的守卫都要重新审视锚点；同型错误在 transform/modify/integrity/标签四处同时出现，证明这是架构升级时的系统性风险模式，不是孤立疏忽。

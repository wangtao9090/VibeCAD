...
- [x] **Step 2: 公开门面**：README 顶部加一行英文简介 + CI badge（`[![CI](https://github.com/wangtao9090/VibeCAD/actions/workflows/ci.yml/badge.svg)]`）+ License badge；pyproject 检查 license/readme 字段。commit。
- [x] **Step 3: 公开化**：`gh repo edit wangtao9090/VibeCAD --visibility public --accept-visibility-change-consequences`；确认 Actions 启用（`gh api repos/wangtao9090/VibeCAD/actions/permissions`）。

## Task 2：CI 三平台闭环（控制者监控 + 修复 agent 迭代）

- [x] **Step 1**: 空 commit 触发或 `gh workflow run`；先盯 **unit job**（五平台快测，~5min）——预期绿（纯 Python）；红则按日志修（多半是平台路径/编码小问题）。
- [x] **Step 2**: **runtime-integration**（ubuntu/macos/windows 真装 2-3GB + 全量 `-m slow`，45min 超时）。已预判 Windows 风险清单（修复 agent 按日志逐一核）：
...
- [x] **Step 3**: 红绿迭代：每轮 `gh run view <id> --log-failed` 取日志 → 派修复 agent（带日志+风险清单）→ 推送 → 重触发。直至三平台 runtime-integration 全绿。**每轮修复必须保持本机 302 fast + 65 slow 全绿**（平台修复不破 mac）。
- [ ] **Step 4**: 用户 Windows 机器手动验证作为 CI 的独立佐证（测试方案附录提供清单，可在 CI 绿后或并行做）。

> **✅ Task 2 实施结论（2026-06-10）**：
> 1. **八轮 CI 卡死的真正根因不是私有仓**：公开化后 run 依然全部 queued——矩阵中的 `macos-13`（Intel runner）已被 GitHub 退役，指向它的 job 永远排队；`runtime-integration` 的 `needs: lint-unit` 等待整个矩阵 → 被无限期卡死。从矩阵删除 macos-13（c2b1e8b）并取消 19 个永不完成的旧 run 后立即疏通。
> 2. **三平台 runtime-integration 一次全绿，零修复**：ubuntu/macos/windows 真装 2-3GB 运行时 + 全量 65 条 slow 全部通过（run 27317570767）。预判的 Windows 风险清单（Library/bin DLL 注入、os.add_dll_directory、micromamba .exe、中文文件名导出编码）全部未触发——R1 的 Windows 适配代码首验即正确。
> 3. 备忘：Actions 提示 checkout@v4/setup-uv@v6 跑在 Node 20（2026-09 移除），届时升 checkout@v5；windows-latest 将于 2026-06-15 重定向到 windows-2025-vs2026。

## Task 3：PyPI 打包与发布通道（可派 agent，与 Task 2 并行）

- [x] **Step 1**: pyproject 完善——version `0.1.0`、`description`（英文一句话）、`readme = "README.md"`、`license`、`authors`、`keywords = ["cad", "freecad", "mcp", "ai"]`、`classifiers`（Python 3.12+/MIT/Topic CAD）、`[project.urls]`（Homepage/Repository）。确认 `[project.scripts] vibecad = ...` 入口无恙。
- [x] **Step 2**: 本地构建验证：`uv build` → `dist/*.whl` + sdist；干净验证 `uvx --from dist/vibecad-0.1.0-py3-none-any.whl vibecad` 握手（stdio 起得来即过，Ctrl-C 退）；`uv run --with dist/...whl python -c "import vibecad; print(vibecad.__version__)"`。
- [x] **Step 3**: `.github/workflows/release.yml`（tag `v*` 触发：uv build → `pypa/gh-action-pypi-publish@release/v1`，OIDC trusted publishing，无 secrets）：
...
- [x] **Step 4**: 用户侧一次性授权指引（写进手册附录与汇报）：PyPI 注册/登录 → Account → Publishing → Add pending publisher（project `vibecad` / owner `wangtao9090` / repo `VibeCAD` / workflow `release.yml` / environment `pypi`）。完成后控制者打 tag `v0.1.0` 即自动发布；发布后干净机 `uvx vibecad` 终验。**未授权前的状态即"一键可发"，不阻塞其余交付。**

> **✅ Task 3 实施结论（2026-06-10）**：用户完成 pending publisher 授权（环境名 `pypi` 已确认填写）→ 打 tag v0.1.0 → release run 27317345220 一次成功 → [pypi.org/project/vibecad](https://pypi.org/project/vibecad/) 上线（whl+sdist）→ 干净环境 `uvx vibecad` 终验：49 包拉取、initialize 握手、tools/list 返回 22 工具全齐。手册/README 已切换主推配置 `uvx vibecad`（39823c4）。已知小项：serverInfo.version 报的是 mcp SDK 版本（1.27.2）而非包版本 0.1.0，不影响功能，下轮顺手修。

## Task 4：用户手册 `docs/USER_GUIDE.md`（派 agent）

- [x] 按 spec §2.4 六章结构成文（中文，面向不懂 CAD/不懂命令行的最终用户，每步给精确命令/话术）：
...
- [x] 「三步接入卡」交付用户（消息形式）：①装 uv（一行命令）②Cowork 加连接器（command/args）③第一句话。等待用户实测反馈，摩擦小项当场修。
- [x] README 链接两份新文档；飞书同步 spec/计划/USER_GUIDE/ACCEPTANCE_TESTS 四份；memory R9 状态；最终汇报（含 PyPI 授权指引与 Windows 验证清单指引）。

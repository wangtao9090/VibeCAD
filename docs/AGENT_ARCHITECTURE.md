...
>
> 当前实现：S3-7 / P0-A 已本地提交；S3-8 宿主 skill、发现合同、0.5.0 分发与最终验收进行中
>
...
| Artifact | FCStd/STEP materialization 与 MCP resource read 已实现 | 能 |
| Host-neutral skill | S3-8 尚未交付 | 尚不能称为顺手可用的宿主产品 |
...
所以准确说法是：**CAD 与任务内核已经打通，公共协议也已切换；S3-8 要把 0.5.0 做到协议与包层
host-ready，让兼容宿主具备低成本发现、正确调用和取回文件的合同。** 未获授权的真实 Claude/Codex
调用仍是 residual，不能把 typed/raw client conformance 宣称成 host-verified。
...
S3-8 skill 必须按服务端 `next_action` 行动：`submit_program` / `provide_input` 提交新 program，
`validate_program` / `reconcile` / `cleanup` 调 `resume_task`，`wait` 先刷新后至多恢复一次，
...
`$HOME/.claude/skills/vibecad-agent` 或 `.claude/skills/vibecad-agent`。S3-8 分别验证这些路径的发现语义。
source、sdist、MCPB 和 standalone archive 会携带同一 skill tree，Python wheel/managed runtime 刻意只
携带 server。无论哪种包，只有复制/安装到宿主识别目录并完成 reload 后才算激活；包内存在不等于
宿主已使用。
...
S3-7 raw stdio E2E 已证明资源 hash、base64 和 FreeCAD reload；AR-1 认为这还不足以代表真实宿主文件
体验。S3-8 必须：

- 在 export tool result 中提供标准 ResourceLink；
- 用 host-neutral skill 明确教授资源读取和 hash/format 检查；
- 完成 packed MCPB 的 typed/raw resource link/read/save 验收；
- 若 Claude/Codex 实际宿主会产生外部模型费用，则在未获授权时诚实保留跨宿主 residual，不能伪造。
...
| 问题 | 结论 |
|---|---|
| Claude/Codex 能否创建、修改、看证据、安全提交 | 内核/协议能；S3-8 补 descriptions、skill、ResourceLink 和分发 E2E 后只能称 host-ready，真实宿主验证仍需授权 |
...
| 下一步先扩 CAD 还是补可靠性 | 先完成 S3-8，再补 P0-B core/G1 前置；P1/P2 不提前 |
...
```text
S3-8
  宿主发现 + skill + ResourceLink + 0.5.0 + final package/E2E
→ P0-B core
  list/recover/cancel/revert + artifact manifest + secure daemon/file grant + source liveness/revocation
  + minimum crash isolation
→ G1 MVP

...
> 状态：Accepted current-state baseline + reviewed 2026-07-20 target architecture
>
> 日期：2026-07-20
>
...
| Stage 3 Agent 主路径与 G0 | S3-R3 已规划 | 控制面、首批直接工具、ModelProgram、durable review 和插件接入缝 |
...
1. 补 command result handle、扩展类型系统和 execution profile metadata。
2. 实现 object/feature UUID、SelectorV1 Level A、per-object observation 和
   preservation verifier。
3. 迁移首批六个 object-level operation。
4. 从 registry 生成直接工具，并证明它们全部进入 Task Kernel。
5. 实现 durable draft/review、Accept/Reject、HEAD CAS 和重启恢复。
6. 在 AR-1 复审一个 Kernel、两种调用面和 Workbench G0 前置是否成立。
7. 组合 Application API、CadExecutionPort、verified export 和 runtime/data 边界。
8. 发布 MCP/skill 契约并执行全量与真实 FreeCAD E2E。

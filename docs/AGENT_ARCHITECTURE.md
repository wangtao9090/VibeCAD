...
> Mechanical V1 已进入 0.7.0；Guided Photo V3 进入 0.8.0；派生参数联动与语义边处理进入 0.9.0；三宿主发布包 Profile 分别验收
>
> MR0 状态：C01..C04 与 conformance gate 已完成；FreeCAD 仍是唯一连接的 CAD adapter，固定
> FCStd/STEP durable 布局保留到 MR1
>
> 发布边界：0.8.0 已发布；0.9.0 是派生参数联动与语义 Fillet/Chamfer 版本
>
...
| 多 runtime 基础 | MR0 C01..C04 与 conformance 已完成 | 不能作为第二 CAD 支持；当前仅 FreeCAD |
...
安装目标与发现语义；0.9.0 发布门继续启动真实 Claude/Codex 主机。source、sdist、MCPB 和 standalone archive
...
| 问题 | 结论 |
|---|---|
| Claude/Codex 能否创建、修改、看证据、安全提交 | 内核/协议能；0.9.0 以同一发布包分别执行真实宿主 smoke，结果不跨宿主外推 |
...
→ [complete] 0.8.0 release
  Guided Photo V3 + native slot + same-candidate three-host package profiles
→ [complete] S41/S42
  derived parameter expressions + semantic Fillet/Chamfer + real FreeCAD reopen/edit/rollback gates
→ 0.9.0 release
  exact S41/S42 package identity + managed runtime + same-candidate host profiles
→ VCAD-A09 parallel routes
  Mechanical S43/S44 + F10 Freeform + M10 metrology + F20 Mesh/SubD
```

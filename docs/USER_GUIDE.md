...
`skills/vibecad-agent/` 或独立资产 `vibecad-agent-skill-0.5.0.zip` 解压得到的同名目录，整体复制
或链接到一个宿主发现路径：
...
当前 `cancel_task` 不会启动 FreeCAD 或 runtime，不会构造 artifact/export 服务，不取得 project
write lease，也不会修改项目 HEAD、源文件或交付目录。它尚不能中断正在验证、执行、提交、准备审核、
接受草案或回滚的任务；这些 active 状态会 fail closed，Worker 终止与取消 reconcile 在 C12 才接通。
如果读取到 `cancel_requested` 或 `cancelling`，只报告取消尚未完成，不要调用当前 `resume_task`
冒充 active-cancel reconciler。MCP `notifications/cancelled` 只取消一次 transport request，不会
持久化 TaskRun 状态。
...
- Workbench UI、可视 preview 或 face/edge 交互选择；
- 通过 `cancel_task` 终止 active FreeCAD execution；当前只支持空闲任务的持久取消；
...
先用 `get_task` 刷新状态与 generation。`awaiting_user_review` 必须调用 `reject_draft`；active 状态返回
`invalid_state` 表示当前 C05 路径没有发出 Worker kill，不能宣称任务已经停止。只有空闲状态进入
`cancelled` 才是已完成的持久取消。不要用 MCP `notifications/cancelled` 代替 `cancel_task`。
...
当前执行 profile 只验证 headless。G1 将在 P0-B core 的认证 daemon、IPC 与 file broker 之上增加
FreeCAD Workbench 的 preview、verdict、Accept/Reject 和 object/feature selection。当前 IPC 仅是
codec seam，不是可供插件连接的可运行协议。

当前外部 Claude/Codex 模型调用尚未纳入本地放行证据。要做宿主实测，必须单独授权相应模型/token
消耗，并记录所用宿主版本、Skill hash、27-tool discovery 与完整任务结果。

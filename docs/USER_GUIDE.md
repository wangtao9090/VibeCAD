...
圆角矩形的公开宽度、高度和圆角半径会驱动内部派生坐标与直线长度；修改这些源参数时，FreeCAD
通过原生表达式同步更新相关 Sketcher 约束，并在同一候选事务内重新验证所有受影响的实体。派生关系
只允许 VibeCAD IR 中有界、同单位、无循环的仿射表达式，不接受任意 FreeCAD 表达式字符串。原子槽
仍保留为可手工编辑的原生数值约束，不参与这套参数联动。
...
hash、39-tool discovery、任务恢复、建模和 ResourceLink/`resources/read` 结果。WorkBuddy 另测其

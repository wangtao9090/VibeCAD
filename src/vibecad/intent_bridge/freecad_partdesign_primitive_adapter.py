"""Private exact PFGv2 lowering for reviewed FreeCAD PartDesign primitives.

All sixteen built-in additive/subtractive primitive operations share one
authority-free plan format and one static rule catalog.  Graph data can select
only complete semantic identities and backend-neutral values; it cannot select
a FreeCAD ``TypeId`` or property name.
"""
...
_ADAPTER_CONTRACT_DOMAIN = b"vibecad.freecad-partdesign-primitive-adapter.v1\0"
_CAPABILITY_DIGEST_DOMAIN = b"vibecad.freecad-partdesign-primitive-capability.v1\0"
_PLAN_DOCUMENT_DIGEST_DOMAIN = b"vibecad.freecad-partdesign-primitive-document.v1\0"
_RECEIPT_DIGEST_DOMAIN = b"vibecad.freecad-partdesign-primitive-lowering-receipt.v1\0"
...
PRIMITIVE_INTENT_DOCUMENT_ROLE_TERM: Final = _bridge_term(
    "role_freecad_parametric_intent", "document-role.parametric-intent"
)
PRIMITIVE_CAPABILITY_DOCUMENT_ROLE_TERM: Final = _bridge_term(
    "role_freecad_partdesign_primitive_capability",
    "document-role.freecad-partdesign-primitive-capability",
)
PRIMITIVE_CAPABILITY_SCHEMA_TERM: Final = _bridge_term(
    "schema_freecad_partdesign_primitive_capability_v1",
    "document-schema.freecad-partdesign-primitive-capability-v1",
)
PRIMITIVE_PLAN_DOCUMENT_ROLE_TERM: Final = _bridge_term(
    "role_freecad_backend_plan", "document-role.freecad-backend-plan"
)
PRIMITIVE_PLAN_SCHEMA_TERM: Final = _bridge_term(
    "schema_freecad_partdesign_primitive_plan_v1",
    "document-schema.freecad-partdesign-primitive-plan-v1",
)

PRIMITIVE_STRUCTURE_TERM: Final = _pfg_term(
    "structure_partdesign_body_feature", "structure.partdesign-body-feature"
)
PRIMITIVE_BASE_ROLE_TERM: Final = _pfg_term("role_base_solid", "input-role.base-solid")
PRIMITIVE_PARAMETERS_ROLE_TERM: Final = _pfg_term(
    "role_primitive_parameters", "input-role.primitive-parameters"
)
PRIMITIVE_SOLID_RESULT_ROLE_TERM: Final = _pfg_term("role_result_solid", "result-role.solid")
PRIMITIVE_SOLID_TYPE_TERM: Final = _pfg_term("type_solid", "value-type.solid")
PRIMITIVE_PARAMETERS_TYPE_TERM: Final = _pfg_term(
    "type_primitive_parameter_set", "value-type.primitive-parameter-set"
)
PRIMITIVE_CANONICAL_JSON_TERM: Final = _pfg_term(
...
_FAMILY_TERMS: Final = {
    family: _pfg_term(f"family_primitive_{family}", f"feature-family.primitive-{family}")
    for family in _FAMILIES
}


@dataclass(frozen=True, slots=True)
class PrimitiveOperationTerms:
    ...
    @property
    def family(self) -> str:
        return self.operation.value.split("_", 1)[1]


PRIMITIVE_OPERATION_TERMS: Final = tuple(
    PrimitiveOperationTerms(
        operation=operation,
        family_term=_FAMILY_TERMS[operation.value.split("_", 1)[1]],
        operation_term=_pfg_term(
            f"operation_{operation.value}",
            f"operation.partdesign-{operation.value.replace('_', '-')}",
        ),
    )
    for operation in PartDesignPrimitiveOperation
)

PRIMITIVE_PFG_TERMS: Final = (
    ...
    *(_FAMILY_TERMS[family] for family in _FAMILIES),
    *(item.operation_term for item in PRIMITIVE_OPERATION_TERMS),
)
...
PRIMITIVE_REQUEST_TERMS: Final = (
    ...
    *(_as_bridge(term) for term in PRIMITIVE_PFG_TERMS),
)
...
_ADAPTER_CONTRACT_SHA256 = hashlib.sha256(
    b"\0".join(
        (
            ...
            PARTDESIGN_PRIMITIVE_RULE_ID.encode("ascii"),
            PARTDESIGN_PRIMITIVE_RULE_CONTRACT_SHA256.encode("ascii"),
            ...
            *(
                ...
                for term in (*PRIMITIVE_REQUEST_TERMS, PFG_SELECTOR_FEATURE_NODE)
            ),
        )
    )
).hexdigest()

FREECAD_PARTDESIGN_PRIMITIVE_ADAPTER_DESCRIPTOR: Final = AdapterDescriptor(
    adapter_id="freecad_partdesign_primitive_adapter",
...
def primitive_capability_payload() -> bytes:
    return _canonical_json(
        {
            "schema_version": 1,
            "authority": "none",
            "adapter": FREECAD_PARTDESIGN_PRIMITIVE_ADAPTER_DESCRIPTOR.to_mapping(),
            "backend": {
                "engine": "FreeCAD",
            ...
            "rule": {
                ...
                "operations": [item.operation.value for item in PRIMITIVE_OPERATION_TERMS],
            },
        }
    )


def build_primitive_capability_document(
    *, artifact_id: str = "artifact_freecad_partdesign_primitive_capability"
) -> tuple[DocumentRef, bytes]:
    payload = primitive_capability_payload()
    digest = hashlib.sha256(_CAPABILITY_DIGEST_DOMAIN + payload).hexdigest()
    return (
        DocumentRef(
            artifact_id=artifact_id,
            role_term_ref_id=PRIMITIVE_CAPABILITY_DOCUMENT_ROLE_TERM.term_ref_id,
            schema_term_ref_id=PRIMITIVE_CAPABILITY_SCHEMA_TERM.term_ref_id,
            document_id=f"freecad_partdesign_primitive_capability_{digest[:32]}",
            document_digest=digest,
            content_sha256=hashlib.sha256(payload).hexdigest(),
            size_bytes=len(payload),
            media_type="application/vnd.vibecad.freecad-partdesign-primitive-capability+json",
        ),
        payload,
    )
...
class LoweredPartDesignPrimitivePlanReceipt:
    request_digest: str
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(self, "receipt_id", f"partdesign_primitive_lowering_{digest[:32]}")
...
def _operation_for_target(
    ...
    if structural is None or _identity(structural) != _identity(PRIMITIVE_STRUCTURE_TERM):
        return None
    matches = tuple(
        item
...
def _build_plan(
    ...
    request_digest: str,
) -> tuple[PartDesignPrimitiveBackendPlan, SubjectRef]:
    if (
    ...
    for expected in PRIMITIVE_PFG_TERMS:
    ...
    expected_ports = {
        _identity(PRIMITIVE_BASE_ROLE_TERM): (
            PRIMITIVE_SOLID_TYPE_TERM,
            "base",
            1 if required_base else 0,
        ),
        _identity(PRIMITIVE_PARAMETERS_ROLE_TERM): (
            PRIMITIVE_PARAMETERS_TYPE_TERM,
            "parameters",
            1,
        ),
    ...
    if (
        ...
        or len(dependencies) not in ({1} if required_base else {0, 1})
        or any(port_kinds.get(item.port_id) != "base" or item.ordinal != 0 for item in dependencies)
    ):
    ...
    if dependencies:
        item = dependencies[0]
        base_node = nodes.get(item.upstream_node_id)
        if (
            base_node is None
            or base_node.body_id != body.body_id
            or base_node.node_id in consumed_nodes
        ):
            _fail(IntentBridgeErrorCode.AUTHORITY_VIOLATION, "/graph/base")
        base_result = _result_with_identity(
            base_node,
            terms,
            role=PRIMITIVE_SOLID_RESULT_ROLE_TERM,
            value_type=PRIMITIVE_SOLID_TYPE_TERM,
            path="/graph/base",
        )
        if item.upstream_result_id != base_result.result_id or len(base_node.results) != 1:
            _fail(IntentBridgeErrorCode.AUTHORITY_VIOLATION, "/graph/base")
        consumed_nodes.add(base_node.node_id)
    ...
    try:
        parameters = PrimitiveParameterSet.from_value(operation.operation, parameter.value.value)
    ...
    plan = PartDesignPrimitiveBackendPlan(
        ...
        adapter_contract_sha256=FREECAD_PARTDESIGN_PRIMITIVE_ADAPTER_DESCRIPTOR.adapter_contract_sha256,
...
def _plan_document(plan: PartDesignPrimitiveBackendPlan) -> DocumentRef:
    ...
    return DocumentRef(
        artifact_id=f"artifact_freecad_partdesign_primitive_plan_{content_sha256[:32]}",
        role_term_ref_id=PRIMITIVE_PLAN_DOCUMENT_ROLE_TERM.term_ref_id,
        schema_term_ref_id=PRIMITIVE_PLAN_SCHEMA_TERM.term_ref_id,
        document_id=f"freecad_partdesign_primitive_plan_{semantic_digest[:32]}",
        document_digest=plan.plan_sha256,
        content_sha256=content_sha256,
        size_bytes=len(payload),
        media_type=PARTDESIGN_PRIMITIVE_PLAN_MEDIA_TYPE,
    )


class FreeCADPartDesignPrimitiveAdapter:
    """One exact PFGv2-to-plan adapter for all reviewed primitive TypeIds."""
    ...
    def descriptor(self) -> AdapterDescriptor:
        return FREECAD_PARTDESIGN_PRIMITIVE_ADAPTER_DESCRIPTOR
    ...
    def lower_with_receipt(
        ...
        *,
        ...
        codecs: TrustedCodecRegistry,
        proof_policy: TrustedProofPolicy,
    ) -> tuple[BackendLoweringResult, LoweredPartDesignPrimitivePlanReceipt]:
        ...
        if any(_identity(term) not in identities for term in PRIMITIVE_REQUEST_TERMS):
        ...
        if (
            ...
            != _identity(PRIMITIVE_INTENT_DOCUMENT_ROLE_TERM)
            ...
            != _identity(PRIMITIVE_CAPABILITY_DOCUMENT_ROLE_TERM)
            or _identity(request_terms[capability_document.schema_term_ref_id])
            != _identity(PRIMITIVE_CAPABILITY_SCHEMA_TERM)
        ):
        ...
        expected_capability = primitive_capability_payload()
        ...
        if (
            ...
            != "application/vnd.vibecad.freecad-partdesign-primitive-capability+json"
            or not hmac.compare_digest(capability_document.document_digest, expected_digest)
            or capability_document.document_id
            != f"freecad_partdesign_primitive_capability_{expected_digest[:32]}"
        ):
        ...
        try:
            ...
            raise
        except (ParametricFeatureGraphError, PartDesignPrimitiveRuleError):
        ...
        if len(plan_payload) > min(
            request.budget.max_output_bytes, MAX_PARTDESIGN_PRIMITIVE_PLAN_BYTES
        ):
        ...
        receipt = LoweredPartDesignPrimitivePlanReceipt(
            request_digest=request.request_digest,
            adapter=self.descriptor,
    ...
    def read_plan(
        self, receipt: LoweredPartDesignPrimitivePlanReceipt
    ) -> tuple[PartDesignPrimitiveBackendPlan, bytes]:
        if (
            type(receipt) is not LoweredPartDesignPrimitivePlanReceipt
            or receipt.adapter != self.descriptor
        ):
            _fail(IntentBridgeErrorCode.AUTHORITY_VIOLATION, "/receipt")
        document = receipt.plan_document
        if (
            document.role_term_ref_id != PRIMITIVE_PLAN_DOCUMENT_ROLE_TERM.term_ref_id
            or document.schema_term_ref_id != PRIMITIVE_PLAN_SCHEMA_TERM.term_ref_id
            or document.media_type != PARTDESIGN_PRIMITIVE_PLAN_MEDIA_TYPE
            or document.size_bytes > MAX_PARTDESIGN_PRIMITIVE_PLAN_BYTES
        ):
            _fail(IntentBridgeErrorCode.INTEGRITY_FAILURE, "/receipt/plan_document")
        try:
            payload = self._sink.read_exact(document, MAX_PARTDESIGN_PRIMITIVE_PLAN_BYTES)
        except (Exception, SystemExit):
            _fail(IntentBridgeErrorCode.INTEGRITY_FAILURE, "/plan_sink")
        if (
...
__all__ = [
    ...
    "LoweredPartDesignPrimitivePlanReceipt",
    "PrimitiveOperationTerms",
    "build_primitive_capability_document",
    "primitive_capability_payload",
]

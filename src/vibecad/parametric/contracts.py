...
MAX_SURFACE_MODIFIERS = 2
MAX_THICKNESS_FACES = 4
MAX_DRAFT_FACES = 8
...
class SemanticFaceRole(StrEnum):
    """One source-feature face role independent of transient FaceN indexes."""
...
class SemanticFaceReference:
    """A face generated by a native Pad section or one profile line sweep."""

    source_feature_id: str
    role: SemanticFaceRole
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(self, "role", _enum(self.role, SemanticFaceRole, "/role"))
        if self.role is SemanticFaceRole.SWEEP:
            if self.geometry_id is None:
                _raise(ParametricErrorCode.INVALID_VALUE, "/geometry_id")
            object.__setattr__(
                self,
                "geometry_id",
                _local_id(self.geometry_id, "/geometry_id", "geometry"),
            )
    ...
    def from_mapping(cls, value: object) -> Self:
        keys = {"schema_version", "source_feature_id", "role", "geometry_id"}
...
class PartDesignFeature:
    ...
    face_targets: tuple[SemanticFaceReference, ...] = ()
    neutral_plane: OriginPlane | None = None
    schema_version: int = PARAMETRIC_SCHEMA_VERSION
    ...
    def __post_init__(self) -> None:
        ...
        if self.sketch_id is None:
            if self.kind not in _SKETCHLESS_FEATURE_KINDS:
            ...
            if self.kind in _SKETCHLESS_FEATURE_KINDS:
                _raise(ParametricErrorCode.INVALID_VALUE, "/sketch_id")
        ...
        if self.kind is FeatureKind.PAD:
            ...
            if extent is not None:
                _raise(ParametricErrorCode.INVALID_VALUE, "/extent")
        elif self.kind is FeatureKind.THICKNESS:
            expected_parameters = {"thickness": DesignUnit.MM}
            if extent is not None:
                _raise(ParametricErrorCode.INVALID_VALUE, "/extent")
        elif self.kind is FeatureKind.DRAFT:
            expected_parameters = {"angle": DesignUnit.DEG}
            if extent is not None:
                _raise(ParametricErrorCode.INVALID_VALUE, "/extent")
        else:
        ...
        if self.kind in _SURFACE_MODIFIER_KINDS and self.symmetric:
            _raise(ParametricErrorCode.INVALID_VALUE, "/symmetric")
        targets = _contract_tuple(
            ...
            "/face_targets",
            maximum=(
                MAX_THICKNESS_FACES if self.kind is FeatureKind.THICKNESS else MAX_DRAFT_FACES
            ),
        )
        if self.kind in _SURFACE_MODIFIER_KINDS:
            if not targets:
                _raise(ParametricErrorCode.INVALID_VALUE, "/face_targets")
            keys = tuple((target.role.value, target.geometry_id) for target in targets)
            if len(set(keys)) != len(keys):
                _raise(ParametricErrorCode.DUPLICATE_ID, "/face_targets")
            if len({target.source_feature_id for target in targets}) != 1:
                _raise(ParametricErrorCode.INVALID_VALUE, "/face_targets")
            if self.kind is FeatureKind.DRAFT and any(
                target.role is not SemanticFaceRole.SWEEP for target in targets
            ):
                _raise(ParametricErrorCode.INVALID_VALUE, "/face_targets")
        elif targets:
            _raise(ParametricErrorCode.INVALID_VALUE, "/face_targets")
        object.__setattr__(self, "face_targets", targets)
        neutral_plane = None
        if self.neutral_plane is not None:
            neutral_plane = _enum(self.neutral_plane, OriginPlane, "/neutral_plane")
        if self.kind is FeatureKind.DRAFT:
            if neutral_plane is None:
                _raise(ParametricErrorCode.INVALID_VALUE, "/neutral_plane")
        elif neutral_plane is not None:
            _raise(ParametricErrorCode.INVALID_VALUE, "/neutral_plane")
        object.__setattr__(self, "neutral_plane", neutral_plane)
    ...
    def to_mapping(self) -> dict[str, object]:
        ...
        elif self.kind in _SURFACE_MODIFIER_KINDS:
            result.update(
                {
                    "face_targets": [item.to_mapping() for item in self.face_targets],
                    "neutral_plane": (
                        None if self.neutral_plane is None else self.neutral_plane.value
                    ),
                }
            )
        return result
    ...
    def from_mapping(cls, value: object) -> Self:
        ...
        surface_keys = {"face_targets", "neutral_plane"}
        optional_keys = pattern_keys | surface_keys
        data = _fields(value, allowed=keys, required=keys - optional_keys)
        ...
        if parsed_kind in _PATTERN_KINDS:
            ...
            unsupported_keys = surface_keys & set(data)
        elif parsed_kind in _SURFACE_MODIFIER_KINDS:
            missing = sorted(surface_keys - set(data))
            if missing:
                _raise(ParametricErrorCode.MISSING_FIELD, join_json_pointer("", missing[0]))
            unsupported_keys = pattern_keys & set(data)
        else:
            unsupported_keys = optional_keys & set(data)
        if unsupported_keys:
            unsupported = sorted(unsupported_keys)[0]
        ...
        return cls(
            ...
            face_targets=_parse_list(
                data.get("face_targets", []),
                SemanticFaceReference,
                "/face_targets",
                maximum=(
                    MAX_THICKNESS_FACES if parsed_kind is FeatureKind.THICKNESS else MAX_DRAFT_FACES
                ),
            ),
...
class ParametricDesignIR:
    ...
    def __post_init__(self) -> None:
        ...
        if pattern_instances > MAX_PATTERN_INSTANCES:
            _raise(ParametricErrorCode.BUDGET_EXCEEDED, "/features")
        surface_modifiers = tuple(item for item in features if item.kind in _SURFACE_MODIFIER_KINDS)
        if (
            len(surface_modifiers) > MAX_SURFACE_MODIFIERS
            or sum(item.kind is FeatureKind.THICKNESS for item in surface_modifiers) > 1
            or sum(item.kind is FeatureKind.DRAFT for item in surface_modifiers) > 1
        ):
            _raise(ParametricErrorCode.BUDGET_EXCEEDED, "/features")
        if surface_modifiers:
            surface_kinds = tuple(item.kind for item in surface_modifiers)
            first_surface = next(
                index for index, item in enumerate(features) if item.kind in _SURFACE_MODIFIER_KINDS
            )
            if (
                features[0].kind is not FeatureKind.PAD
                or first_surface != 1
                or len(features) != 1 + len(surface_modifiers)
                or surface_kinds
                not in {
                    (FeatureKind.DRAFT,),
                ...
                _raise(ParametricErrorCode.INVALID_ORDER, f"/features/{first_surface}/kind")
        ...
        for index, feature in enumerate(features):
            ...
            if feature.kind in _PATTERN_KINDS:
                ...
                continue
            if feature.kind in _SURFACE_MODIFIER_KINDS:
                require_evidence(feature.evidence_ids, f"/features/{index}/evidence_ids")
                source_ids = {target.source_feature_id for target in feature.face_targets}
                if len(source_ids) != 1:
                    _raise(ParametricErrorCode.INVALID_VALUE, f"/features/{index}/face_targets")
                source_id = next(iter(source_ids))
                source = declared_features.get(source_id)
                if source is None:
                    _raise(
                        ParametricErrorCode.UNKNOWN_REFERENCE,
                        f"/features/{index}/face_targets/0/source_feature_id",
                    )
                if source.kind is not FeatureKind.PAD or (
                    feature.kind is FeatureKind.DRAFT and source.symmetric
                ):
                    _raise(
                        ParametricErrorCode.INVALID_ORDER,
                        f"/features/{index}/face_targets/0/source_feature_id",
                    )
                source_sketch = sketch_by_id[source.sketch_id or ""]
                geometry_by_id = {item.id: item for item in source_sketch.geometries}
                for target_index, target in enumerate(feature.face_targets):
                    if target.role is not SemanticFaceRole.SWEEP:
                        continue
                    geometry = geometry_by_id.get(target.geometry_id or "")
                    target_path = f"/features/{index}/face_targets/{target_index}/geometry_id"
                    if geometry is None:
                        _raise(ParametricErrorCode.UNKNOWN_REFERENCE, target_path)
                    if geometry.kind is not GeometryKind.LINE or geometry.construction:
                        _raise(ParametricErrorCode.INVALID_VALUE, target_path)
                for name, parameter_id in feature.parameters.items():
                    parameter = parameter_by_id.get(parameter_id)
                    path = f"/features/{index}/parameters/{name}"
                    if parameter is None:
                        _raise(ParametricErrorCode.UNKNOWN_REFERENCE, path)
                    expected_unit = (
                        DesignUnit.DEG if feature.kind is FeatureKind.DRAFT else DesignUnit.MM
                    )
                    if parameter.unit is not expected_unit or parameter.value <= 0:
                        _raise(ParametricErrorCode.INVALID_VALUE, path)
                    if feature.kind is FeatureKind.DRAFT and parameter.value > 30:
                        _raise(ParametricErrorCode.INVALID_VALUE, path)
                declared_features[feature.id] = feature
                continue

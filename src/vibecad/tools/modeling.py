...
def new_document(
    ...
    *,
    ...
    if session.doc is not None and session.is_dirty() and not discard_unsaved:
        raise ValueError(
            "当前项目有未保存修改；请先调用 save_project，或明确传 discard_unsaved=true 后再新建"
        )
...
def add_box(
    ...
    *,
    ...
    with session._transaction("add_box", part=part):
        with silence_fd1():
...
def add_cylinder(
    ...
    *,
    ...
    with session._transaction("add_cylinder", part=part):
        with silence_fd1():
            import FreeCAD  # noqa: PLC0415

...
the 124-operation, seven-facet matrix is a maintainer/CI conformance gate, not
...
CURRENT_MANAGED_VERIFICATION_RECEIPT_COUNT: Final = 19
CURRENT_MANAGED_VERIFICATION_FORMAL_OPERATION_COUNT: Final = 124
CURRENT_MANAGED_VERIFICATION_PROMOTION_OPERATION_COUNT: Final = 104
CURRENT_MANAGED_VERIFICATION_NATIVE_TYPE_COUNT: Final = 102

_CURRENT_REVIEWED_FAMILY_COUNT: Final = 11
...
def _collect_current_receipts(*, freecad: object) -> tuple[ReviewedVerificationReceipt, ...]:
    ...
    receipts = (core, curves, *part_b, *wave_c, *wave_d, *legacy)
...
def build_current_managed_freecad_reviewed_verification_set_for_maintainers(
    *,
    ...
    """Run the exact current 124-by-seven maintainer/CI verification matrix.

    The caller must inject the already authenticated, headless managed FreeCAD
    module.  All nineteen family verifiers execute sequentially against that

"""Private Reviewed product callbacks for three PartDesign patterns.

The legacy pattern adapter already owns the complete semantic graph contract,
including content-bound Body-origin axis/plane locators.  This module projects
that exact contract through the shared Reviewed-family seam and authenticates
the two ordered, same-run Body features before entering the native rule.
"""
...
_OWNERSHIP_DIGEST_DOMAIN = b"vibecad.partdesign-pattern-ownership.v1\0"
_MANIFEST_BUILD_ID: Final = hashlib.sha256(
    b"FreeCAD-build\0" + PARTDESIGN_PATTERN_FREECAD_ENGINE_BUILD_ID.encode("ascii")
).hexdigest()
...
_NATIVE_TYPES: Final = MappingProxyType(
    {
...
_NATIVE_PROPERTIES: Final = MappingProxyType(
    {
...
PARTDESIGN_PATTERN_REVIEWED_OPERATIONS: Final = tuple(PartDesignPatternOperation)
PARTDESIGN_PATTERN_OPERATION_SPECS: Final = tuple(
    ReviewedOperationSpec(
        operation_id=terms.operation.value,
        semantic_term=_bridge_term(terms.operation_term),
        native_type_id=_NATIVE_TYPES[terms.operation],
        native_operation="apply_partdesign_pattern_plan",
        native_property_names=_NATIVE_PROPERTIES[terms.operation],
    )
    for terms in PATTERN_OPERATION_TERMS
)

PARTDESIGN_PATTERN_MANIFEST: Final = FamilyBatchManifest(
    ...
    operations=PARTDESIGN_PATTERN_OPERATION_SPECS,
    max_plan_bytes=MAX_PARTDESIGN_PATTERN_PLAN_BYTES,
)

_OPERATIONS_BY_ID: Final = MappingProxyType(
    {item.operation_id: item for item in PARTDESIGN_PATTERN_MANIFEST.operations}
)
PARTDESIGN_PATTERN_REVIEWED_PRODUCT_IDENTITIES: Final = tuple(
    (
        f"{PARTDESIGN_PATTERN_MANIFEST.family_id}.{operation.value}",
        _OPERATIONS_BY_ID[operation.value].semantic_term.term_id,
    )
    for operation in PARTDESIGN_PATTERN_REVIEWED_OPERATIONS
)


class PartDesignPatternSourceRole(StrEnum):
    BASE = "base"
    SOURCE_FEATURE = "source_feature"


PARTDESIGN_PATTERN_REQUIRED_SOURCE_ROLES: Final = MappingProxyType(
    {
        operation.value: (
            PartDesignPatternSourceRole.BASE,
            PartDesignPatternSourceRole.SOURCE_FEATURE,
        )
        for operation in PARTDESIGN_PATTERN_REVIEWED_OPERATIONS
    }
)
...
def resolve_partdesign_pattern_reviewed_operation(
    operation_id: object,
    semantic_operation: object,
) -> ReviewedOperationSpec | None:
    """Resolve only one exact public formal identity."""

    if type(operation_id) is not str or type(semantic_operation) is not str:
        return None
    try:
        index = PARTDESIGN_PATTERN_REVIEWED_PRODUCT_IDENTITIES.index(
            (operation_id, semantic_operation)
        )
    except ValueError:
        return None
    operation = PARTDESIGN_PATTERN_REVIEWED_OPERATIONS[index]
...
def _build_reviewed_plan(
    ...
    if manifest is not PARTDESIGN_PATTERN_MANIFEST:
        _integrity_failure()
    try:
...
def _validate_plan_contract(
    ...
    if (
        ...
        _integrity_failure()
    try:
        decoded = decode_partdesign_pattern_backend_plan(
...
def validate_partdesign_pattern_reviewed_plan(
    plan: object,
    receipt: ReviewedPlanReceipt,
    operation: ReviewedOperationSpec,
) -> None:
    """Bind an exact canonical pattern plan to its Reviewed route receipt."""

    if (
        type(receipt) is not ReviewedPlanReceipt
        or receipt.operation != operation
        or receipt.manifest_sha256 != PARTDESIGN_PATTERN_MANIFEST.manifest_sha256
        or receipt.adapter != PARTDESIGN_PATTERN_MANIFEST.adapter
    ):
        _integrity_failure()
...
def partdesign_pattern_reviewed_adapter_factory(
    sink: PlanSink,
) -> ExactReviewedFamilyAdapter:
    return ExactReviewedFamilyAdapter(
        ...
        decode_plan=decode_partdesign_pattern_backend_plan,
        validate_binding=validate_partdesign_pattern_reviewed_plan,
    )
...
def _selection_receipt(
    source: object,
    selection: PatternObjectSelection,
    *,
    body_id: str,
) -> None:
    receipt = source.native_receipt
    if (
        getattr(receipt, "body_id", None) != body_id
...
def _source_body(document: object, source_objects: tuple[object, ...]) -> object:
    try:
        bodies = tuple(
            item
            ...
            and all(
                any(source is child for child in tuple(item.Group)) for source in source_objects
            )
        )
    except (Exception, SystemExit):
        _integrity_failure()
...
def _authenticated_bindings(
    ...
    *,
    ...
    selections = (plan.base, plan.source_feature)
    source_objects = tuple(item.object for item in source_results)
    for source, selection, item in zip(source_results, selections, source_objects, strict=True):
        try:
        ...
        if (
            ...
            _integrity_failure()
        _selection_receipt(source, selection, body_id=plan.body_id)

    if (plan.base == plan.source_feature) != (source_objects[0] is source_objects[1]):
        _integrity_failure()
    body = _source_body(document, source_objects)
    try:
        if body.Tip is not source_objects[0]:
            raise ValueError
    except (AttributeError, ValueError):
        _integrity_failure()
    return PartDesignPatternExecutionBindings(
        document=document,
        body=body,
        body_id=plan.body_id,
        base=AuthenticatedPatternObject(
            object=source_objects[0],
            node_id=plan.base.node_id,
            result_id=plan.base.result_id,
        ),
        source_feature=AuthenticatedPatternObject(
            object=source_objects[1],
...
class PartDesignPatternOwnershipClosure:
    """Family-owned receipt used for adoption and later same-run selection."""

    native_receipt: PartDesignPatternConformanceReceipt
    body_id: str
    node_id: str
    result_id: str
    native_type_id: str
    ...
    def __post_init__(self) -> None:
        if type(self.native_receipt) is not PartDesignPatternConformanceReceipt:
            _integrity_failure()
        operation = self.native_receipt.operation
        if (
            operation not in PARTDESIGN_PATTERN_REVIEWED_OPERATIONS
            or self.native_type_id != _NATIVE_TYPES[operation]
    ...
    @property
    def operation(self) -> PartDesignPatternOperation:
        ...
        try:
            valid = (
                ...
                and math.isclose(
                    ...
                    rel_tol=0.0,
                    abs_tol=max(1e-9, self.native_receipt.after_volume_mm3 * 1e-12),
                )
            )
        except (Exception, SystemExit, TypeError, ValueError, OverflowError):
            valid = False
...
def execute_partdesign_pattern_reviewed_plan_with_sources(
    document: object,
    plan: object,
    payload: bytes,
    plan_document: DocumentRef,
    ...
    *,
    ...
    """Apply one pattern using only two sealed ordered same-run results."""
    ...
    try:
        ...
        _integrity_failure()
    if (
        type(receipt) is not PartDesignPatternConformanceReceipt
        ...
        or any(
            actual is not expected
            for actual, expected in zip(after_group[: len(before_group)], before_group, strict=True)
        )
        or result is not after_group[-1]
        or bindings.body.Tip is not result
        or result.BaseFeature is not bindings.base.object
        or tuple(result.Originals) != (bindings.source_feature.object,)
        or getattr(result, "TypeId", None) != operation.native_type_id
        or not _valid_solid(result)
    ):
        _integrity_failure()
    owned_receipt = PartDesignPatternOwnershipClosure(
...
def execute_partdesign_pattern_reviewed_plan(
    document: object,
    plan: object,
    payload: bytes,
    plan_document: DocumentRef,
    ...
    if type(context) is not _ReviewedFamilyExecutionContext or context.document is not document:
        _integrity_failure()
    return execute_partdesign_pattern_reviewed_plan_with_sources(
        document,
        plan,
        payload,
        plan_document,
...
class PartDesignPatternReviewedFamilySpec:
    manifest: FamilyBatchManifest
    ...
    def __post_init__(self) -> None:
        if (
            self.manifest is not PARTDESIGN_PATTERN_MANIFEST
            or self.subject_type_term != _bridge_term(PATTERN_STRUCTURE_TERM)
            ...
            != tuple(item.value for item in PARTDESIGN_PATTERN_REVIEWED_OPERATIONS)
...
PARTDESIGN_PATTERN_REVIEWED_FAMILY_SPEC: Final = PartDesignPatternReviewedFamilySpec(
    manifest=PARTDESIGN_PATTERN_MANIFEST,
    subject_type_term=_bridge_term(PATTERN_STRUCTURE_TERM),
    adapter_factory=partdesign_pattern_reviewed_adapter_factory,
    validate_plan=validate_partdesign_pattern_reviewed_plan,
    execute_plan=execute_partdesign_pattern_reviewed_plan,
    operation_ids=tuple(item.value for item in PARTDESIGN_PATTERN_REVIEWED_OPERATIONS),
    minimum_sources=2,
    maximum_sources=2,
)
...
__all__ = [
    "PARTDESIGN_PATTERN_MANIFEST",
    "PARTDESIGN_PATTERN_OPERATION_SPECS",
    ...
    "PartDesignPatternReviewedFamilySpec",
    "PartDesignPatternSourceRole",
    "execute_partdesign_pattern_reviewed_plan",
    ...
    "validate_partdesign_pattern_reviewed_plan",
]

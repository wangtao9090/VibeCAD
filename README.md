...
MCPB manifest 与运行时投影同一份冻结合同，当前公开 27 个工具。每个工具都有简短说明、严格输入
...
`cancel_task` 首次调用必须使用刚读取的 task generation。它只会把 `created`、`needs_plan`、
`program_ready` 或 `needs_input` 空闲任务以 CAS 立即持久化为 `cancelled`；取消响应未知时可重放
完全相同的请求，并取得同一取消结果。等待审核的 draft 必须用 `reject_draft`，其他 active 状态在
当前路径中 fail closed；终止正在运行的 FreeCAD Worker 和后续 reconcile 属于 C12。
...
发布资产中的 `vibecad-agent-skill-0.5.0.zip` 解压后只有一个顶层 `vibecad-agent/` 目录，可整体复制
...
0.5.0 Beta 的 S3-8 基线已经在本地验证 MCP 协议、Skill 包结构、FCStd/STEP ResourceLink 和受管
FreeCAD E2E，因此基线包可以称为 protocol/package `host-ready`。当前 C05 分支的 27-tool discovery
与 Skill 合同是未发布源码候选；完整 0.6.0 package refresh 属于 C14，不能提前宣称已重新打包放行。
当前阶段也没有消费用户的 Claude/Codex 外部模型配额执行第二宿主验收，所以不能称为
`host-verified`；实际跨宿主模型调用仍是独立残项。

## 架构边界与路线

当前层次是 MCP transport → Agent application → Task Kernel → CAD execution port → FreeCAD runtime。
当前 IPC 只是版本化 codec seam，并非可供独立 Workbench 使用的可运行协议。

S3-8 已完成本地 protocol/package host-ready 闭环；当前后续顺序为
P0-B core → G1 → P0-B hardening → P1/G2 → P2：

- **P0-B core（执行中）**：任务/项目/版本发现、文件级比较、只读交付清单和空闲任务取消已进入
  当前分支；verified forward revert、daemon、认证 IPC、file broker、source liveness 与可终止
  FreeCAD Worker 继续按后续 packet 实现；
- **G1**：FreeCAD Workbench 中的 preview、verdict、Accept/Reject 与 object/feature selection；
- **P1/G2**：Sketcher/PartDesign、受控导入、单零件生产能力和手工 checkpoint；
- **P2**：装配、BOM、TechDraw、制造发布与企业交付链。

G1 Workbench 尚未交付，当前主路径仍是受管 Agent/headless 执行。当前也不支持 face/edge 选择、
Workbench 交互、STEP/STL import、照片重建或 simulation。

进一步阅读：[用户手册](docs/USER_GUIDE.md)、[验收测试](docs/ACCEPTANCE_TESTS.md)、
[整体架构](docs/ARCHITECTURE.md)、[Agent 架构](docs/AGENT_ARCHITECTURE.md) 和
[产品能力路线图](docs/PRODUCT_CAPABILITY_ROADMAP.md)。

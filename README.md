...
| `render_part(view="iso", annotate=None, edges_of=None)` | **PNG 预览图**（view: iso\|front\|top\|right\|back）。`annotate="faces"` 出**面标注图**（A/B/C 标签+尺寸线）+ 标签表；`annotate="edges"` 出边标注图（E1…，`edges_of="A"` 只画 A 面的边） |
| `add_hole(face, diameter, depth=None, offset=[u,v])` | **在指定面打圆孔**（face=面标签）；depth 省略=通孔；offset 面内毫米偏移（省略=面正中） |

...
| `add_box(length, width, height)` | 参数化长方体（Part::Box，mm） |
| `add_cylinder(radius, height)` | 参数化圆柱（Part::Cylinder，mm） |
| `boolean_cut(base_name, tool_name)` | 布尔差集（Part::Cut）：从 base 减去 tool |
| `export_part(output_dir, fmt="all")` | 导出 STEP/STL/glTF（fmt: step\|stl\|gltf\|both\|all） |

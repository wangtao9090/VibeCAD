...
宿主 Agent（用户自带模型）
  ...
  → 生成候选 revision 与验证证据
  → auto_commit 自动发布，或 require_review 由用户接受/拒绝草案
  → 导出并读取校验过的 FCStd / STEP artifact
```

这个流程的目标是服务“从已有样品、草图或模型继续修改”的主要场景；当前只能从空项目，或只含
受支持 `Part::Box` / `Part::Cylinder` 对象的 FCStd 导入信封开始。任意 FCStd、照片/视频到网格、
照片到 STL、2D 草图识别、STEP/STL 导入、逆向工程和仿真尚未接入，未来可以作为外部引擎或受控
导入器连接；VibeCAD 聚焦中间的可编辑 CAD 编排，而不是自研这些底层引擎。
...
进入 artifact catalog 的 PUBLISHED 交付物通过只读 MCP resource URI 返回。资源描述绑定项目、
revision、任务代数、manifest、内容哈希和大小；候选草案只有在状态与验证证据完全匹配时才可导出。
...
- 项目创建、TaskRun、草案和导出都有持久化记录；但 `create_task` 尚无 request key，响应结果未知时
  目前不能找回其 task id，必须停止并报告，不能盲重试。
...
[产品能力路线图](docs/PRODUCT_CAPABILITY_ROADMAP.md)。当前实现分层见[整体架构](docs/ARCHITECTURE.md)，
产品与宿主边界见 [Agent 架构](docs/AGENT_ARCHITECTURE.md)。

## 推进路线

AR-1 已完成，当前顺序是 S3-8 → P0-B core → G1 → P0-B hardening → P1/G2 → P2：

- **G1**：基于 P0-B core 已交付的认证 daemon/file broker，实现 FreeCAD Workbench、候选版本可视化
  审核和 object/feature 交互；
...
- 当前交付格式为 FCStd/STEP；导入只接受 Box/Cylinder-only FCStd 信封，通用 FCStd、STEP/STL
  导入、逆向转换和照片重建尚未接入；
- 当前没有仿真、碰撞求解或制造工艺验证；

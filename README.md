...
MCPB manifest 与运行时投影同一份冻结合同，当前公开 24 个工具。每个工具都有简短说明、严格输入
...
| 项目与版本 | `create_project`, `get_project`, `list_projects`, `list_revisions` |
| 任务与草案 | `create_task`, `list_tasks`, `get_task`, `get_task_events`, `submit_model_program`, `resume_task`, `accept_draft`, `reject_draft` |
...
0.5.0 Beta 在本地验证 MCP 协议、24-tool discovery、Skill 包结构、FCStd/STEP ResourceLink 和受管
FreeCAD E2E，因此可以称为 protocol/package `host-ready`。当前阶段没有消费用户的 Claude/Codex
外部模型配额执行第二宿主验收，所以不能称为 `host-verified`；实际跨宿主模型调用仍是独立残项。
...
- **P0-B core**：本地 daemon、认证 IPC、file broker、source liveness 与可终止进程；

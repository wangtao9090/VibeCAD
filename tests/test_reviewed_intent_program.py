...
def test_reviewed_intent_program_is_canonical_content_bound_and_registry_closed() -> None:
    ...
    assert metadata.resource_budget.max_created_objects == 43
...
def test_reviewed_primitive_route_table_is_exact_and_closed() -> None:
    ...
    assert len(REVIEWED_IMAGEPLANE_ROUTES) == 1
    assert len(REVIEWED_PART_RESIDUAL_ROUTES) == 8
    assert len(REVIEWED_PART_DRESSUP_ROUTES) == 3
    assert len(REVIEWED_PARTDESIGN_RESIDUAL_ROUTES) == 3

...
def test_current_specs_build_five_packs_and_promote_all_thirty_one_type_ids() -> None:
    ...
    assert len(packs) == 5
    assert sum(len(item.entries) for item in packs) == 31
    ...
    assert len(projection.manifest.formal_bindings) == 31

...
def test_pending_uninstall_runs_before_respawn(sup_factory, tmp_path):
    ...
    home = tmp_path / "home"
    (home / "mamba").mkdir(parents=True)          # 强哨兵：确属我们的安装产物
    ...
    assert not home.exists()                       # spawn 前清理已执行，home 整目录被删
...
def test_spawn_runs_pending_uninstall_before_popen(monkeypatch):
    ...
    order: list[str] = []
    monkeypatch.setattr(
        supervisor.uninstall, "perform_pending_uninstall",
...
def test_spawn_real_cmd_uninstall_marker_falls_back_to_bootstrap(monkeypatch, tmp_path):
    ...
    assert not home.exists()                       # 待删标记已兑现，home 整目录消失
...
def test_eof_during_swap_reaps_new_child(unclosed_stdin, monkeypatch):
    ...
    monkeypatch.setattr(sup, "_replay_handshake",
                        lambda ch: sup._client_eof.set() or True)
    reaped: list = []
...
@pytest.mark.skipif(sys.platform == "win32",
                    reason="Windows terminate 已是强杀，无 kill 升级路径")

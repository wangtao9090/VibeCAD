...
from vibecad.parametric import freecad_part_file_import_rules as import_rules
...
def test_staged_lease_adopts_the_handle_canonical_path(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    alias_root = Path("C:/alias-root")
    alias_directory = alias_root / "stage"
    alias_path = alias_directory / "artifact.step"
    canonical_root = Path("C:/canonical-root")
    canonical_directory = canonical_root / "stage"
    canonical_path = canonical_directory / "artifact.step"
    canonical_by_alias = {
    ...
    lease = import_rules._StagedImportLease(alias_directory, alias_path)  # noqa: SLF001

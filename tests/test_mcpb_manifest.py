...
def test_packaged_readme_describes_the_current_agent_first_surface():
    ...
    for required in (
        ...
        "real VLM-backed photo reconstruction, and simulation are not currently supported",
        "S20.5 provides only the deterministic-fake interface and lifecycle",
        "create_reconstruction",
        "direct WorkBuddy attachment ingress has not been verified",
        "31-tool discovery",
        "WorkBuddy (verified)",
    ):
    ...
    for required in (
        ...
        "真实 VLM 驱动的照片重建或 simulation",
        "S20.5 只提供 deterministic-fake 接口与生命周期",
        "create_reconstruction",
        "WorkBuddy 直接附件入口尚未验证",
        "31-tool discovery",
        "WorkBuddy（已验证）",
    ):
